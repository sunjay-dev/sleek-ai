  ...
  return (
    <aside className="fixed md:relative h-dvh w-54 bg-white border-r border-secondary flex flex-col z-30">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-secondary">
        ...
        <button onClick={() => setCollapsed(true)} className="flex items-center justify-center h-8 w-8 rounded-lg hover:bg-[#e9e9e980] transition" title="Collapse sidebar">
      ...
      <div className="px-3 pt-3 pb-2 border-b border-secondary">
        <button onClick={createChat} className="w-full px-3 py-2 rounded-lg hover:bg-[#e9e9e980] transition flex items-center gap-2 text-xs font-medium text-primary">
      ...
      <nav className="px-3 pt-3 pb-3 overflow-y-auto grow min-h-0">
        {chats.map((chat) => {
          ...
          return (
            <button key={chat.id} onClick={() => selectChat(chat.id)} className={`w-full text-left px-3 py-2 my-1 rounded-lg text-xs transition ${isActive ? "bg-primary text-primary" : "hover:bg-[#e9e9e980] text-primary"}`}>
      ...
      <div onClick={openClerkMenu} className="border-t border-secondary px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-[#e9e9e980] transition">
        <div className="text-sm font-medium text-primary truncate">{user?.fullName ?? user?.username ?? "User"}</div>
